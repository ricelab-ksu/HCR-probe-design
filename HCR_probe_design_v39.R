#!/usr/bin/env Rscript
# ============================================================================
# HCR PROBE DESIGNER (v34 - cross-species edition)
# ============================================================================
...
#      matching genomic.fna and genomic.gtf are also present, each gene gets an
#      isoform-structure track plot, and the "shared exons" option can design
#      against exons common to all isoforms.
#      (The "species" column in your CSV should be e.g. "melanogaster",
#      "simulans", "eugracilis", "erecta", "ananassae", "teissieri".)
#
#      Supported species are detected automatically: any subfolder named
#      d_<species> containing an rna.fna(.gz) appears in the app. To add a
#      species, download its RefSeq files into a new subfolder, e.g.:
...
#      plus genomic.fna.gz and genomic.gtf.gz for structure plots/shared CDS).
...
# Hardcoded reference file path - edit this if you move the folder.
# The folder should contain subfolders like d_melanogaster, d_simulans, d_eugracilis,
# each with an rna.fna file inside.
DATA_DIRS <- "/Users/ricelab/Desktop/bioinformatics/reference_files"

# For debugging: print the path being used
...
# Download rna.fna.gz (+ genomic.fna.gz/genomic.gtf.gz when asked) for a
# species into DATA_DIRS/d_<species>. Returns a status message.
download_species_ref <- function(species, with_genome = TRUE,
  ...
  base <- paste0("https://ftp.ncbi.nlm.nih.gov/genomes/all/GCF/", segs[1],
                 "/", segs[2], "/", segs[3], "/", gcf, "/")
  dest <- file.path(DATA_DIRS, paste0("d_", sp))
  dir.create(dest, recursive = TRUE, showWarnings = FALSE)
  files <- "rna.fna.gz"
  if (with_genome) files <- c(files, "genomic.fna.gz", "genomic.gtf.gz")
  for (f in files) {
    url <- paste0(base, gcf, "_", f)
    out <- file.path(dest, f)                  # save under the plain name
    progress(paste0("Downloading ", sp, " ", f, " ..."))
    ok <- tryCatch({
    ...
    if (!ok) {
      ...
      if (f == "rna.fna.gz")
        return(paste0("Download failed for ", sp, " (", f, "). Check your ",
                      "internet connection, or download manually from ", base))
      progress(paste0("  (", f, " not available for ", sp, "; continuing)"))
    }
  }
  paste0(sp, " reference saved to ", dest)
}
...
read_fasta_from_accession <- function(acc) {
  fetch <- function(a) {
    ...
    suppressWarnings(tryCatch(readLines(url, warn = FALSE), error = function(e) character(0)))
  }
  fa <- fetch(acc)
...
ncbi_fetch_gene <- function(gene, organism) {
  ...
  esearch <- function(term) {
    ...
    xml <- tryCatch(suppressWarnings(readLines(url, warn = FALSE)),
                    error = function(e) NULL)
    if (is.null(xml)) stop("Could not reach NCBI.")
  ...
  efetch_records <- function(ids) {
    fa <- tryCatch(suppressWarnings(
      readLines(paste0(base, "efetch.fcgi?db=nuccore&id=", paste(ids, collapse = ","),
                       "&rettype=fasta&retmode=text"), warn = FALSE)),
      error = function(e) NULL)
...
parse_gtf_exons <- function(gtf_path, gene_symbol, cache = NULL) {
  ...
  # Match by gene symbol in attributes (column 9)
  # NCBI GTF uses gene_name, gene_id, or gene
...
find_gtf_file <- function(species) {
  ...
  subs <- subs[grepl(key, tolower(basename(subs)), fixed = TRUE)]
  for (sd in subs) {
    hits <- list.files(sd, pattern = "(^|_)genomic\\.gtf(\\.gz)?$",
                       ignore.case = TRUE, full.names = TRUE)
    if (length(hits)) return(hits[1])
  }
  NULL
...
plot_combined_species_tracks <- function(res) {
  ...
  prev_gi <- 0
  for (t in tracks) {
    if (t$gi != prev_gi) { cursor <- cursor - if (prev_gi == 0) 1 else gap + 1
                           prev_gi <- t$gi }
  ...
  mtext("dark blocks = designed probe binding sites (selected transcript per species)",
        side = 3, line = 0, cex = 0.6, col = "gray40", adj = 0)
}
...
design_gene <- function(job, settings, index, cache) {
  ...
  tryCatch({
    ...
    if (isTRUE(settings$relax_filters) && length(cands) < settings$pairs_per_gene) {
      ...
      cands2 <- find_candidates(rec$seq, settings$spacing, gc_min2, gc_max2, hp2, exon_bounds,
                                stride = stride)
        ...
        for (round in 1:3) {
          ...
          if (round == 3) {
            res$warnings <- c(res$warnings, paste0(
              ...
              ") but no cleaner windows were available."))
            break
          }
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      fluidRow(
        ...
        column(6, selectInput("default_amp", "Default amplifier",
                              c("Auto (rotate B1-B5)" = "auto", AMP_LIST)))
      ),
      fluidRow(
      ...
      fluidRow(
        column(6, numericInput("temp_c", "Temperature (\u00b0C)", 37)),
        column(6, numericInput("na_m", "Na+ (M)", 0.3, min = 0.01, step = 0.1))
      ),
      ...
      checkboxGroupInput("check_species", "Check probes in other species",
                         ...
                         selected = setdiff(SPECIES_CHOICES, "melanogaster")),
      ...
      div(style = "background:#f1f5f9; border-radius:6px; padding:8px 10px; margin-bottom:8px;",
        ...
        selectInput("dl_species", NULL,
                    choices = setdiff(names(SPECIES_GCF), SPECIES_CHOICES),
                    width = "100%"),
        checkboxInput("dl_genome_too",
                      "Also download genome + GTF (large, ~50-150 MB; needed for structure plots)",
                      TRUE),
        actionButton("dl_species_btn", "Download from NCBI",
                     class = "btn-default btn-sm"),
        textOutput("dl_species_status", inline = FALSE)
      ),
      helpText("After designing, each probe's two 25-nt binding halves are ",
               "matched against the same gene's transcript in these species, ",
               "so you can see which pairs are reusable across species. Uses ",
               "local rna.fna files (or NCBI when the fallback above is on). ",
               "Uncheck ALL boxes to design for the CSV's species only, with ",
               "no cross-species check (e.g. a teissieri-only run)."),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$dl_species_btn, {
    sp <- input$dl_species
    req(sp)
    msg <- withProgress(message = paste("Downloading", sp), value = 0.1, {
      download_species_ref(sp, isTRUE(input$dl_genome_too),
                           progress = function(m)
                             setProgress(0.5, detail = m))
    })
    dl_species_status(msg)
    new_choices <- detect_species()
    updateCheckboxGroupInput(session, "check_species", choices = new_choices,
                             selected = union(input$check_species,
                                              setdiff(new_choices, "melanogaster")))
    updateSelectInput(session, "dl_species",
                      choices = setdiff(names(SPECIES_GCF), new_choices))
  })
  ...
  observeEvent(input$run, {
    ...
    settings <- list(pairs_per_gene = input$pairs_per_gene, spacing = input$spacing,
                     gc_min = input$gc_min, gc_max = input$gc_max, max_hp = input$max_hp,
                     default_amp = input$default_amp, ncbi_fallback = input$ncbi_fallback,
                     relax_filters = input$relax_filters,
                     use_shared_exons = input$use_shared_exons,
                     avoid_splice_junctions = input$avoid_splice_junctions,
                     use_thermo = input$use_thermo, filter_thermo = input$filter_thermo,
                     temp_c = input$temp_c, na_m = input$na_m, oligo_conc = 5e-5,
                     tm_min = input$tm_min, tm_max = input$tm_max,
                     dg_min = input$dg_min, dg_max = input$dg_max,
                     target_dg = input$target_dg, target_dg_half = input$target_dg_half,
                     comp_a = input$comp_a, comp_c = input$comp_c,
                     comp_stack = input$comp_stack, comp_cstack = input$comp_cstack,
                     offtarget_screen = input$offtarget_screen,
                     ot_seed = input$ot_seed, max_offtarget = input$max_offtarget)
  ...
  output$summary <- renderUI({
    ...
    wellPanel(
      ...
      span(
        downloadButton("dl_order", "Oligo order CSV", class = "btn-primary btn-sm"),
        downloadButton("dl_all_amp", "HCR PROBE MAKER excel sheet (Shows probes with all-amplifier combos)", class = "btn-default btn-sm"),
  ...
  output$dl_pools <- downloadHandler(
    ...
    content = function(file) {
      rows <- do.call(rbind, lapply(pools(), function(p) data.frame(
        Pool = p$index, Genes = paste(p$genes, collapse = "; "),
        Total_bp = p$bp, stringsAsFactors = FALSE)))
      rows <- rbind(rows, data.frame(Pool = "TOTAL", Genes = "",
                                     Total_bp = total_oligos() * 45))
  ...
  output$dl_shared_cds_fasta <- downloadHandler(
    ...
    content = function(file) {
      ...
      if (!length(ok)) {
        writeLines(">no_data\nNo shared CDS data available.", file)
        return()
      }
      lines <- character(0)
      for (r in ok) {
        # Extract each shared region and concatenate with NNNNNN spacers
        region_seqs <- character(nrow(r$shared_exons))
...
No shared CDS sequences extracted."
  ...
  # ---- All-amplifier table download (Excel-style: one row per target, all B1-B5) ----
  output$dl_all_amp <- downloadHandler(
    filename = function() "HCR_all_amplifier_probes.csv",
    content = function(file) {
...
default_settings <- function() list(
  ...
  relax_filters = TRUE, use_shared_exons = TRUE, avoid_splice_junctions = TRUE,
...
#   Rscript HCR_probe_design_v34.R my_genes.csv
run_batch <- function(csv_path, cross = TRUE) {
  ...
  for (p in pools) for (g in p$genes)
    pool_of[[strsplit(g, " ")[[1]][1]]] <- p$index
  order <- do.call(rbind, unlist(lapply(results, function(r)
    lapply(r$pairs, function(p) data.frame(
      Pool = { v <- pool_of[[r$gene]]; if (is.null(v)) "" else v },
      Name = c(p$odd_name, p$even_name),
      Sequence = c(p$odd, p$even), stringsAsFactors = FALSE))),
  ...
  if (!is.null(amp_tbl))
    utils::write.csv(amp_tbl, file.path(out_dir, "HCR_probe_maker_all_amplifiers.csv"),
                     row.names = FALSE)

  # --- Cross-species conservation table ---
  cs <- cross_species_rows(results)
  if (!is.null(cs) && nrow(cs))
    utils::write.csv(cs, file.path(out_dir, "HCR_cross_species.csv"),
                     row.names = FALSE)
  ...
  if (length(pools)) {
    rows <- do.call(rbind, lapply(pools, function(p) data.frame(
      Pool = p$index, Genes = paste(p$genes, collapse = "; "),
      Total_bp = p$bp, stringsAsFactors = FALSE)))
    utils::write.csv(rows, file.path(out_dir, "HCR_pool_summary.csv"),
                     row.names = FALSE)
  }

  # --- Run parameters ---
  writeLines(c(paste0("HCR probe design batch run: ", Sys.time()),
  ...
  # --- Plots (one PDF page per gene: isoform view + cross-species view) ---
...
if (sys.nframe() == 0L) {
  ...
  if (!is.na(csv_arg) && file.exists(csv_arg)) {
    run_batch(csv_path = csv_arg, cross = !("--no-cross" %in% args))
    quit(save = "no")
  }
  cat("\n============================================================\n")
  ...
  if (dir.exists(DATA_DIRS)) {
    ...
    if (length(subs)) {
      ...
      for (d in subs) {
        ...
        has_gtf <- length(list.files(d, pattern = "(^|_)genomic\\.gtf(\\.gz)?$", ignore.case = TRUE)) > 0
        cat("   -", basename(d), ":",
            if (has_rna) "rna.fna OK" else "NO rna.fna!",
            "|", if (has_gen) "genomic.fna OK" else "no genomic.fna",
            "|", if (has_gtf) "genomic.gtf OK" else "no genomic.gtf", "\n")
      }
    } else {
